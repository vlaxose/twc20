function [S, Lt_opt, kappa, converged, rate_approximation_error] = proposed(H,W,F_RF,F_BB,Ns,Lt,maxDBiters, bits_vec, noise_variance, p_dac, Pt, Pcp, Pps, check_rate_approximation_flag, rho)
  ...
  beta = sqrt(pi*sqrt(3)./(2*(1-diag(Delta).^2)));
  
  [Ur,Er] = eig(R_eta);
  a = real(diag(1./diag(sqrt(Er))))*Ur*W'*H*F_RF*Delta;
  b = sqrt(rho*1/Ns)*F_BB.';
  F = F_RF*F_BB;
  ...
  for iter=1:maxDBiters 
    ...
    cvx_begin quiet
      variable s(Lt)
      weighted_norm = Nt*Pt/Pcp;
      for i=1:Lt
        weighted_norm = weighted_norm + s(i)*((sqrt(pi*sqrt(3)/(2*(1-Delta(i,i)^2)))*p_dac+Nt*Pps)/Pcp +  rho*0.01*trace(F_RF(i, :)'*F_RF(i, :)));
      end
      Q = zeros(Ns);
      for i=1:Lt
    ...
    weighted_norm = Nt*Pt/Pcp;
    for i=1:Lt
      weighted_norm = weighted_norm + S(i,i)*((sqrt(pi*sqrt(3)/(2*(1-Delta(i,i)^2)))*p_dac+Nt*Pps)/Pcp +  rho*0.01*trace(F(i, :)'*F(i, :)));
