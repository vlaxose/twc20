function [ee, rate, power] = ee_computation(S, noise_variance, bits_vec, Lt_opt, Ns, W, H, F_RF, F_BB, Delta, p_dac, Pt, Pps, Pcp, rho)
    ...
    rate = real(log2(det(eye(Ns) + rho*1/Ns*pinv(R_eta)*W'*H*F_RF*Delta*S*(F_BB*F_BB')*S*Delta'*F_RF'*H'*W)));
    
    % Consumed power
    F = F_RF*S*F_BB;
    power = (2*trace(S*diag(2.^bits_vec))*p_dac + Nt*Pt + Nt*Lt_opt*Pps)/Pcp + rho*0.01*trace(F*F');
